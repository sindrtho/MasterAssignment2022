...
# at "https://github.com/olalium/face-reconstruction/blob/master/Networks/mobilenet_v2.py"
...
def decoder_network(inputs):
	x_e = Conv2DTranspose(
		512, 4, strides=1, padding='same', activation=keras_backend.relu)(inputs)  # 8 x 8 x 512

	x_e = Conv2DTranspose(
		256, 4, strides=2, padding='same', activation=keras_backend.relu)(x_e)  # 16 x 16 x 256
	
	x_e = Conv2DTranspose(
		128, 4, strides=2, padding='same', activation=keras_backend.relu)(x_e)  # 32 x 32 x 128
	
	x_e = Conv2DTranspose(
		64, 4, strides=2, padding='same', activation=keras_backend.relu)(x_e)  # 64 x 64 x 64

	x_e = Conv2DTranspose(
		32, 4, strides=2, padding='same', activation=keras_backend.relu)(x_e)  # 128 x 128 x 32
	
	x_e = Conv2DTranspose(
		16, 4, strides=1, padding='same', activation=keras_backend.relu)(x_e)  # 128 x 128 x 16

	x_e = Conv2DTranspose(
		8, 4, strides=1, padding='same', activation=keras_backend.relu)(x_e)  # 128 x 128 x 8
	x_e = Conv2DTranspose(
			3, 4, strides=1, padding='same', activation=keras_backend.tanh)(x_e) # 128 x 128 x 3
...
def get_keras_model(input_shape, N=2**14, alpha=1.0, squeeze=False):
	...
	x_e = ConvLSTM2D(
		first_filters, kernel_size=(3,3), strides=(2, 2), padding='same', return_sequences=True)(inputs)
	...
	x_e = _inverted_residual_block(x_e, 160, (3, 3), t=6, alpha=alpha, strides=2, n=2, squeeze=squeeze)
