...
def get_keras_model(base_filters, input_shape, N=27508):  # 30247 is the lowest amount of points available across all given ground truths and is the number of points sampled from all other ground truths.
        ...
        # decoder
        x_e = Conv2DTranspose(base_filters * 32, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 8 x 8 x 512

        x_e = Conv2DTranspose(base_filters * 16, 4, strides=(2, 2), padding='same', activation=keras_backend.relu)(x_e)  # 16 x 16 x 256
        x_e = Conv2DTranspose(base_filters * 16, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 16 x 16 x 256
        x_e = Conv2DTranspose(base_filters * 16, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 16 x 16 x 256

        x_e = Conv2DTranspose(base_filters * 8, 4, strides=(2, 2), padding='same', activation=keras_backend.relu)(x_e)  # 32 x 32 x 128
        x_e = Conv2DTranspose(base_filters * 8, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 32 x 32 x 128
        x_e = Conv2DTranspose(base_filters * 8, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 32 x 32 x 128

        x_e = Conv2DTranspose(base_filters * 4, 4, strides=(2, 2), padding='same', activation=keras_backend.relu)(x_e)  # 64 x 64 x 64
        x_e = Conv2DTranspose(base_filters * 4, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 64 x 64 x 64
        x_e = Conv2DTranspose(base_filters * 4, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 64 x 64 x 64

        x_e = Conv2DTranspose(base_filters * 2, 4, strides=(2, 2), padding='same', activation=keras_backend.relu)(x_e)
        x_e = Conv2DTranspose(base_filters * 2, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 128 x 128 x 32
        x_e = Conv2DTranspose(base_filters * 2, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 128 x 128 x 32

        # x_e = Conv2DTranspose(base_filters, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 256 x 256 x 16
        # x_e = Conv2DTranspose(base_filters, 4, strides=(1, 1), padding='same', activation=keras_backend.relu)(x_e)  # 256 x 256 x 16

        x_e = Conv2DTranspose(3, 4, strides=(1, 1), padding='same', activation=keras_backend.tanh)(x_e)
        x_e = Conv2DTranspose(3, 4, strides=(1, 1), padding='same', activation=keras_backend.tanh)(x_e)
        x_e = Conv2DTranspose(3, 4, strides=(1, 1), padding='same', activation=keras_backend.tanh)(x_e)   # 256x256x3

        x_e = Reshape((2**14, 3))(x_e)
